...
class MateInOneSpec extends Specification {
  ...
  "Board" should {
    ...
    "be immutable" in
      { Board.initial.move(G2->G3) must beSome.which { b => b.move(G3->G4); onlyTheseMoved(Vector(Piece.piece(White, Pawn, G3)))(b) } }

  }

  "Simple moves" should {
    "1. g3"                                   in movesAllowed(G2->G3)(Vector(Piece.piece(White, Pawn, G3)))
    "1. Nf3"                                  in movesAllowed(G1->F3)(Vector(Piece.piece(White, Knight, F3)))
    "1. g3 a6 2. Bh3"                         in movesAllowed(G2->G3, A7->A6, F1->H3)(Vector(Piece.piece(White, Pawn, G3), Piece.piece(White, Bishop, H3), Piece.piece(Black, Pawn, A6)))
    "1. d4 e5 2. dxe5 d6 3. Bg5 dxe5 4. Bxd8" in movesAllowed(D2->D4, E7->E5, D4->E5, D7->D6, C1->G5, D6->E5, G5->D8)(Vector(Piece.piece(White, Bishop, D8), Piece.piece(Black, Pawn, E5)), nCaptured = 3)
  }
  ...
  "Pawn moves" should {
    ...
    "pawn promotion"                               in movesAllowed(G2->G4, H7->H5, H2->H4, H5->G4, H4->H5, H8->H6, F2->F3, H6->G6, H5->H6, G4->F3, H6->H7, F3->E2, H7->H8->Queen)(Vector(Piece.piece(White, Queen, H8), Piece.piece(Black, Rook, G6), Piece.piece(Black, Pawn, E2)), nCaptured = 3)
    "pawn promotion on capture"                    in movesAllowed(G2->G4, H7->H5, H2->H4, H5->G4, H4->H5, H8->H6, F2->F3, H6->G6, H5->H6, G4->F3, H6->H7, F3->E2, H7->G8->Queen)(Vector(Piece.piece(White, Queen, G8), Piece.piece(Black, Rook, G6), Piece.piece(Black, Pawn, E2)), nCaptured = 4)
  ...
  "En passant" should {
    ...
    "1. e4 e6 2. e5 d5 3. exd6; White captures the black pawn en passant." in
      movesAllowed(E2->E4, E7->E6, E4->E5, D7->D5, E5->D6)(Vector(Piece.piece(White, Pawn, D6), Piece.piece(Black, Pawn, E6)), nCaptured = 1)
  ...
  "Castling" should {
    ...
    "1. Nf3 Nf6 2. g3 g6 3. Bh3 Bh6 4. O-O O-O; White and black castle kingside." in
      movesAllowed(G1->F3, G8->F6, G2->G3, G7->G6, F1->H3, F8->H6, `O-O`, `O-O`)(Vector(Piece.piece(White, Knight, F3),  Piece.piece(White, Pawn, G3),  Piece.piece(White, Bishop, H3),  Piece.piece(White, King, G1),  Piece.piece(White, Rook, F1),  Piece.piece(Black, Knight, F6),  Piece.piece(Black, Pawn, G6),  Piece.piece(Black, Bishop, H6),  Piece.piece(Black, King, G8),  Piece.piece(Black, Rook, F8)))

    "1. Nc3 Nc6 2. d3 d6 3. Bg5 Bg4 4. Qd2 Qd7 5. O-O-O O-O-O; White and black castle queenside." in
      movesAllowed(B1->C3, B8->C6, D2->D3, D7->D6, C1->G5, C8->G4, D1->D2, D8->D7, `O-O-O`, `O-O-O`)(Vector(Piece.piece(White, Knight, C3),  Piece.piece(White, Pawn, D3),  Piece.piece(White, Bishop, G5),  Piece.piece(White, Queen, D2),  Piece.piece(White, Rook, D1),  Piece.piece(White, King, C1),  Piece.piece(Black, Knight, C6),  Piece.piece(Black, Pawn, D6),  Piece.piece(Black, Bishop, G4),  Piece.piece(Black, Queen, D7),  Piece.piece(Black, Rook, D8),  Piece.piece(Black, King, C8)))
  ...
  "Check" should {
    ...
    "1. e4 Nf6 2. d4 Ng4 3. c4 Nxh2 4. f3 Nxf3+ 5. Nxf3; Capture the checking piece." in
      movesAllowed(E2 -> E4, G8 -> F6, D2 -> D4, F6 -> G4, C2 -> C4, G4 -> H2, F2 -> F3, H2 -> F3, G1 -> F3)(Vector(Piece.piece(White, Pawn, C4), Piece.piece(White, Pawn, D4), Piece.piece(White, Pawn, E4), Piece.piece(White, Knight, F3)), nCaptured = 3)

    "1. e4 e5 2. d4 Bb4+ 3. Ke2; Move the White king out of check." in
      movesAllowed(E2 -> E4, E7 -> E5, D2 -> D4, F8 -> B4, E1 -> E2)(Vector(Piece.piece(White, King, E2), Piece.piece(White, Pawn, D4), Piece.piece(White, Pawn, E4), Piece.piece(Black, Bishop, B4), Piece.piece(Black, Pawn, E5)))

    "1. e4 e5 2. d4 Bb4+ 3. c3; Block the check." in
      movesAllowed(E2 -> E4, E7 -> E5, D2 -> D4, F8 -> B4, C2 -> C3)(Vector(Piece.piece(White, Pawn, C3), Piece.piece(White, Pawn, D4), Piece.piece(White, Pawn, E4), Piece.piece(Black, Bishop, B4), Piece.piece(Black, Pawn, E5)))
  ...
  def movesAllowed(moves: MoveBase*)(movedPieces: Vector[Piece] = Vector(), nCaptured: Int = 0) = {
  ...
  def onlyTheseMoved(movedPieces: Vector[Piece] = Vector(), nCaptured: Int = 0): Board => Result = {
    val initialPieces = Board.initial.pieces
    (board) => {
      val stationaryPieces = board.pieces.filter(initialPieces.contains)
      stationaryPieces.size + movedPieces.size + nCaptured must beEqualTo(32)
      initialPieces must containAllOf(stationaryPieces)
      board.pieces must containAllOf(movedPieces)
    }
  }

...
object Piece {
  def withSide(pieces: Set[Piece], side: Side) = pieces.filter(_.side == side) // TODO these can be simpler! so much boilerplate
  ...
  def withoutSquare(pieces: Set[Piece], square: Square) = pieces.filterNot(_.square == square)
